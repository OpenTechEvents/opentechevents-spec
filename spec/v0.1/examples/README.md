...
| [`event-from-ics.json`](event-from-ics.json) | Evento **importado de un `.ics`**: sin `url`, con `source` y **sin `attendanceMode`** — porque iCalendar no sabe expresarlo, y callarse es más honesto que inventar. |
...
`event-meetup.json` y `event-conference-cfp.json` usan campos que **NO forman parte de la v0.1**:

| Campo | Estado |
| --- | --- |
| `tags` | 🗣️ En discusión — [issue #5](https://github.com/OpenTechEvents/opentechevents-spec/issues/5) |
| `cfp` | 🗣️ En discusión |
